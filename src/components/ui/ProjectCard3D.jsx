...
export const ProjectCard3D = ({ project, index }) => {
  ...
  return (
    <motion.div 
      ref={cardRef} onMouseMove={handleMouseMove} onMouseLeave={() => { x.set(0); y.set(0); }} 
      style={{ perspective: 1000 }} 
      initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} 
      viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.5, delay: index * 0.2 }} className="h-full"
    >
      <motion.div style={{ rotateX, rotateY }} className="group relative h-full rounded-3xl overflow-hidden bg-slate-900 border border-slate-800 hover:border-blue-500/50 transition-colors duration-300">
        ...
        <div className="relative z-20 p-8 -mt-20 flex flex-col h-[calc(100%-11rem)]">
          ...
          <p className="text-slate-400 mb-8 leading-relaxed flex-grow">{project.desc}</p>
          
          <div className="flex gap-6 mt-auto">
              {/* RENDERIZAÇÃO CONDICIONAL: Só aparece se project.github existir e não for vazio */}
              {project.github && (
                <a href={project.github} target="_blank" rel="noopener noreferrer" className="text-slate-300 hover:text-white flex items-center gap-2 text-sm font-bold uppercase tracking-widest transition-all hover:translate-x-1">
              ...
              {project.demo && (
                <a href={project.demo} target="_blank" rel="noopener noreferrer" className="text-slate-300 hover:text-white flex items-center gap-2 text-sm font-bold uppercase tracking-widest transition-all hover:translate-x-1">
